...
var notes = []Note{
	...
	},
	{
		...
		Lines: []string{
			...
			"NOTIFY_ON keeps every notification channel quiet on clean runs and speaking up on warnings",
		},
		Actions: []string{
			"To be notified only when something is wrong, set NOTIFY_ON=warning (warnings and failures) in backup.env",
