...
func TestWebhookNotifier_sendToEndpoint_DiscordContent(t *testing.T) {
	logger := logging.New(types.LogLevelDebug, false)
	data := createTestNotificationData()
	...
	t.Run("custom content", func(t *testing.T) {
		notifier.client = &http.Client{
			Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
				...
				if _, ok := payload["embeds"]; !ok {
					t.Fatalf("expected embeds in payload")
				}
				return &http.Response{
					StatusCode: http.StatusOK,
					Body:       io.NopCloser(strings.NewReader("ok")),
		...
		endpoint := config.WebhookEndpoint{
			...
			Method: "POST",
			Format: "discord",
			Discord: config.WebhookDiscordOptions{
				ContentEnabled: true,
				Content:        "Hello",
			},
		}
	...
	t.Run("auto summary when content empty", func(t *testing.T) {
		notifier.client = &http.Client{
			Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
				body, readErr := io.ReadAll(req.Body)
				if readErr != nil {
				...
				if !ok || strings.TrimSpace(content) == "" {
					t.Fatalf("expected non-empty content summary, got %v", payload["content"])
				}
				if !strings.Contains(content, data.Hostname) {
					t.Fatalf("expected content to contain hostname %q, got %q", data.Hostname, content)
				}
				return &http.Response{
					StatusCode: http.StatusOK,
					Body:       io.NopCloser(strings.NewReader("ok")),
	...
	t.Run("truncates content over 2000 chars", func(t *testing.T) {
		...
		notifier.client = &http.Client{
			Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
				...
				if utf8.RuneCountInString(content) != 2000 {
					t.Fatalf("content rune length = %d; want 2000", utf8.RuneCountInString(content))
				}
				if !strings.HasSuffix(content, "…") {
					t.Fatalf("expected content to end with ellipsis, got %q", content[len(content)-10:])
				}
				return &http.Response{
					StatusCode: http.StatusOK,
					Body:       io.NopCloser(strings.NewReader("ok")),
