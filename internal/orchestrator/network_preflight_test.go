...
func TestRunNetworkPreflightValidationPrefersIfquery(t *testing.T) {
	fake := &fakeCommandRunner{
		outputs: map[string][]byte{
			"ifquery --check -a": []byte("ok\n"),
		},
	}
	...
	if result.Tool != "ifquery" {
		t.Fatalf("tool=%q want %q", result.Tool, "ifquery")
	}
}
...
func TestRunNetworkPreflightValidationFallsBackWhenFlagsUnsupported(t *testing.T) {
	fake := &fakeCommandRunner{
		outputs: map[string][]byte{
			"ifquery --check -a": []byte("ifquery: unrecognized option '--check'\n"),
			"ifup --no-act -a":   []byte("ifup: unknown option --no-act\n"),
			"ifup -n -a":         []byte("ok\n"),
		},
		errs: map[string]error{
			"ifquery --check -a": errors.New("exit status 2"),
			"ifup --no-act -a":   errors.New("exit status 2"),
		},
	}

	available := func(name string) bool {
		return name == "ifquery" || name == "ifup"
	}
