...
var postInstallComponentDescriptions = map[string]string{
	...
	"BACKUP_PVE_BACKUP_FILES":       "PVE vzdump backup-job definition files. PVE-only; disable if you run no scheduled guest backups.",
	...
	"BACKUP_USER_CONFIGS":           "Per-user configuration under home directories. Disable if there are no user accounts to preserve.",
