...
func run() int {
	...
	backupResult := runBackupMode(backupModeOptions{
		ctx:              ctx,
		cfg:              cfg,
		logger:           logger,
		envInfo:          envInfo,
		unprivilegedInfo: unprivilegedInfo,
		updateInfo:       updateInfo,
		toolVersion:      toolVersion,
		dryRun:           dryRun,
		startTime:        startTime,
		heapProfilePath:  heapProfilePath,
		serverIDValue:    serverIDValue,
		serverMACValue:   serverMACValue,
	})
	orch = backupResult.orch
	earlyErrorState = backupResult.earlyErrorState
	pendingSupportStats = backupResult.supportStats
	return finalize(backupResult.exitCode)
}
