...
func TestMaybeAutoMigrateDaemon_Gates(t *testing.T) {
	...
	run := func(t *testing.T, configPath, baseDir string, origin schedulerModeOrigin) string {
		t.Helper()
		return captureStdout(t, func() {
			maybeAutoMigrateDaemon(context.Background(), configPath, baseDir, "/usr/local/bin/proxsave", origin, logging.NewBootstrapLogger())
		})
	}
	...
	t.Run("already on daemon: no migration", func(t *testing.T) {
		...
		out := run(t, cfgPath, dir, schedulerModeOriginInjected)
	...
	t.Run("key already present: honoured, no migration", func(t *testing.T) {
		...
		out := run(t, cfgPath, dir, schedulerModeOriginConfigured)
		if strings.Contains(out, migratingMarker) {
	...
	t.Run("provenance unknown: no migration", func(t *testing.T) {
		...
		out := run(t, cfgPath, dir, schedulerModeOriginUnknown)
		if strings.Contains(out, migratingMarker) {
	...
	t.Run("unreadable config: silent no-op", func(t *testing.T) {
		...
		out := run(t, missing, dir, schedulerModeOriginInjected)
		if strings.Contains(out, migratingMarker) {
