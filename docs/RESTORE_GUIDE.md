...
- `proxsave --cleanup-guards` (preview with `--dry-run`) unmounts bind-mount guards **and** clears the recorded `chattr +i` immutable flags, but only on mountpoints that are **not currently mounted** (clearing a live mount would touch the wrong inode). The guard directory is kept until nothing is pending.
