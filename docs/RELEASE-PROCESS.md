...
Two repository rulesets back this process. They live in the GitHub repository settings, not in this repo, so they are recorded here.

| Ruleset | Target | Rules | What it prevents |
|---|---|---|---|
| `main: release gates` | `refs/heads/main` | required status check `release-notes-guard` | merging a release PR whose version has no what's-new entry |
| `tags: v* immutable` | `refs/tags/v*` | restrict deletions, restrict updates, block force pushes | moving, retagging, or deleting a published version tag |

Neither has bypass actors, so both apply to repository admins as well.

`tags: v* immutable` deliberately does NOT restrict CREATION, because `post-merge-release.yml` has to create the `vX.Y.Z` tag once. Its pattern is `v*` and not `*`, so the `pr-v*` trigger tags stay freely creatable and deletable, which `release-intake.yml` relies on to clean up after itself.

One consequence is worth stating plainly: once a `vX.Y.Z` tag exists it cannot be moved or removed. If a release fails after the tag was created, that version number is spent and the fix ships as the next one.

## Screen 0 what's-new notes

After an upgrade the dashboard opens on Screen 0 to show what changed. Its copy lives in the `notes` slice in `internal/whatsnew/registry.go`, one entry per FINAL release. The authoring rules (what belongs in `Lines`, what belongs in `Actions`, the fixed section headers) are documented in that file, above the slice.

Write the entry BEFORE the first tag of a line, beta included, not in the final's release PR. Notes are keyed to the final version and the seen-flag is compared on FINALIZED versions, so `0.31.0-beta1` and `0.31.0` are the same line. A beta released without the entry renders the empty state and still records the line as seen, which suppresses Screen 0 when the final lands: the users who tested the betas are exactly the ones who never see that release's notes.

The gate enforces this. `TestReleaseNotesPresent` finalizes the requested tag, so `pr-v0.31.0-beta1` demands the `0.31.0` entry just as `pr-v0.31.0` does.
...
- Push `dev` to origin.
- Starting a NEW `X.Y.Z` line, including its first beta? Add the `X.Y.Z` what's-new entry to `internal/whatsnew/registry.go` before you tag. See [Screen 0 what's-new notes](#screen-0-whats-new-notes).
...
`release-notes-guard.yml` runs alongside it. It lints the what's-new registry on every internal dev -> main PR and, when a release marker is present, requires the entry for the released line. It is a REQUIRED status check on `main`: a red `release-notes-guard` blocks the merge button, it is not advisory. The other checks are informative.
...
- Do not tag a release, beta included, before its what's-new entry exists in `internal/whatsnew/registry.go`.
